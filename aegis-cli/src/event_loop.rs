...
pub fn spawn_event_loops(bpf: &mut aya::Ebpf, ctx: EventLoopContext) -> Result<(), anyhow::Error> {
    ...
    let _logging_cfg = ctx.logging_cfg;
    let webhooks_cfg = ctx.webhooks_cfg;
    let pcap_on = ctx.pcap_on;

    // Take ownership of EVENTS (IPv4/TC shared ring buffer)
    ...
    let events_ipv6 = RingBuf::try_from(events_ipv6_map)?;
    ...
    let blocklist_inner = blocklist_arc.clone();
    let ban_count_inner = ban_count_atomic.clone();

    tokio::spawn(async move {
        let mut events = match AsyncFd::new(events) {
            Ok(events) => events,
            Err(e) => {
                let _ = log_tx_inner.try_send(format!("EVENTS ringbuf async setup failed: {}", e));
                return;
            }
        };

        loop {
            let mut guard = match events.readable_mut().await {
                Ok(guard) => guard,
                Err(e) => {
                    let _ = log_tx_inner.try_send(format!("EVENTS ringbuf poll failed: {}", e));
                    break;
                }
            };
            ...
            while let Some(item) = ring.next() {
                if item.len() < mem::size_of::<PacketLog>() {
                    continue;
                }
                let log = unsafe { ptr::read_unaligned(item.as_ptr() as *const PacketLog) };
    ...
    tokio::spawn(async move {
        let mut events_ipv6 = match AsyncFd::new(events_ipv6) {
            Ok(events) => events,
            Err(e) => {
                let _ =
                    log_tx_v6.try_send(format!("EVENTS_IPV6 ringbuf async setup failed: {}", e));
                return;
            }
        };

        loop {
            let mut guard = match events_ipv6.readable_mut().await {
                Ok(guard) => guard,
                Err(e) => {
                    let _ = log_tx_v6.try_send(format!("EVENTS_IPV6 ringbuf poll failed: {}", e));
                    break;
                }
            };
            ...
            let ring = guard.get_inner_mut();
            while let Some(item) = ring.next() {
                if item.len() < mem::size_of::<PacketLogIpv6>() {
                    continue;
                }
                let log = unsafe { ptr::read_unaligned(item.as_ptr() as *const PacketLogIpv6) };
